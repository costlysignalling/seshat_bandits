...
tiff("distance_visual.tif",width=10,height=10,units="cm",res=600,compression="lzw")
...
text(show,c(3,4,2),labs,pos=4,col=cold2)
lines(rep(0,2),CId,col=alpha(cold,0.8),lwd=2)
points(0,mud,col=cold,pch=16)
rect(0.4,5.3,9.5,6.5,col=alpha("white",0.8),border=NA)

rect(show[2]+0.3,3.6,show[2]+10,4.5,col=alpha("white",0.8),border=NA)
rect(show[1]+0.3,2.6,show[1]+7,3.5,col=alpha("white",0.8),border=NA)
rect(show[3]+0.3,1.6,show[3]+11,2.5,col=alpha("white",0.8),border=NA)

text(0,mud,"1 + Extra bonus for the\ndirectly preceding polity\nwithin the same NGA",col=cold,pos=4)
dev.off()
