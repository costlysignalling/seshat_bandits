...
```{r}
raw<-read.csv("D:/Seshat_Bandits/equinox2020_dataset_seshat.csv",stringsAsFactors=F,encoding="UTF-8")
...
Dataset: considering only multistate variables (14:15) and  binary variable (16:128)
And Test of Dataset
```{r}
########Dataset
...
#It gives the warning message for KMO

```
I checked in the dataset and the longwall variable as it has zero variance, thus delete this variable form the dataset
...
```{r}
######Parallel Analysis to determine the number of Factors needed
nofactors <- fa.parallel(BinaryTable, n.obs = 428, fm="ml", fa="fa") #maximum likelyhood
nofactors

```
Results on number of factors:
#based on eigen value  12 possible factors 
#based on Parallel analysis, it suggests us 16 factors

Performing the Factor Analysis by considering the 12 possible factors. 
```{r}
###### Factor Analysis
fa12 <- fa(r = BinaryTable, 
          nfactors = 12,
...
```{r}
print(fa12$loadings, cutoff = 0.3, sort = TRUE)
#At above mention cutoff it omit 31 rows
```
